...
import { CustomSummaryAgent, isCustomSummarySelected, isCustomSummaryAvailable } from '../../CustomSummaryAgent.js';
...
export class SessionRoutes extends BaseRouteHandler {
  ...
  private getActiveAgent(): SDKAgent | GeminiAgent | OpenRouterAgent | CustomSummaryAgent {
    if (isCustomSummarySelected()) {
      if (isCustomSummaryAvailable()) {
        logger.debug('SESSION', 'Using Custom Summary agent');
  ...
  private startGeneratorWithProvider(
    ...
    const agent = provider === 'custom' ? this.customSummaryAgent :
                  (provider === 'openrouter' ? this.openRouterAgent :
                  (provider === 'gemini' ? this.geminiAgent : this.sdkAgent));
    const agentName = provider === 'custom' ? 'CustomSummary' :
